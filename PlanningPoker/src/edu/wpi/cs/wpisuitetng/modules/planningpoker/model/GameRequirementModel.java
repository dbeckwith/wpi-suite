...
public class GameRequirementModel extends AbstractModel {
	...
	public float getEstimateMedian() {
		...
		if (estimates.isEmpty()) {
			...
			final List<Estimate> estimates_copy = new ArrayList<>(estimates);
			Collections.sort(estimates_copy);
			final int count = estimates_copy.size();
			if (estimates_copy.size() % 2 == 1) {
				toReturn = estimates_copy.get((int) (count / 2.0))
						.getEstimate();
			} else {
				toReturn = (estimates_copy.get((int) (count / 2.0))
						.getEstimate() + estimates_copy.get(
						(int) (count / 2.0 - 1.0)).getEstimate()) / 2;
			}
		}
		return toReturn;
	}
