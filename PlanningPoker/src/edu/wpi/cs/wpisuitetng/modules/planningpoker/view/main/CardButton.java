...
public class CardButton extends JPanel implements MouseListener, ChangeListener {
	...
	public CardButton() {
		...
		final JFormattedTextField txt = ((JSpinner.NumberEditor) input
	...
	public void paintComponent(Graphics g) {
		...
		if (isEnabled()) {
			...
			if (!textInput) {
				...
				final Rectangle2D r = g2.getFontMetrics().getStringBounds(
						value, g);
				...
				if (maxInput != DeckModel.NO_LIMIT) {
					...
					final String maxString = "Max : "
							+ cardFormat.format(((SpinnerNumberModel) input
									.getModel()).getMaximum());
					g2.setFont(g.getFont().deriveFont(
							getWidth() * FONT_SIZE * 0.3f));
					final Rectangle2D r = g2.getFontMetrics().getStringBounds(
							maxString, g);
					g2.drawString(
							maxString,
	...
	public void setLimit(int limit) {
		maxInput = limit;
		final double cardLimit = (limit == DeckModel.NO_LIMIT) ? Double.MAX_VALUE
				: limit;
