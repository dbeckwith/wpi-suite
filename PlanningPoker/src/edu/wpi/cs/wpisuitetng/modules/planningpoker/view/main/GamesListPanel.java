...
public class GamesListPanel extends javax.swing.JPanel {
    ...
    private void updateTree() {
        ...
        // if the selected node is a requirement, remember which game it's in
        GameModel selectedReqGame = null;
        if (gameTree.getSelectionCount() != 0) {
            TreePath selectionPath = gameTree.getSelectionPath();
            selectedNodeUserObject = ((DefaultMutableTreeNode) selectionPath
                    .getLastPathComponent()).getUserObject();
            if (selectedNodeUserObject instanceof GameRequirementModel) {
                selectedReqGame = (GameModel) ((DefaultMutableTreeNode) ((DefaultMutableTreeNode) selectionPath
        ...
        final DefaultMutableTreeNode rootNode = new DefaultMutableTreeNode() {
            ...
            
            {
                ...
                for (GameModel gm : GameListModel.getInstance().getGames()) {
                    ...
                    completeFolder.setUserObject("Complete Games ("
        ...
        while (treeEnum.hasMoreElements()) {
            node = (DefaultMutableTreeNode) treeEnum.nextElement();
            if (node.getUserObject() != null) {
                if (selectedReqGame != null
                        && selectedNodeUserObject != null
                        && selectedNodeUserObject instanceof GameRequirementModel
                        && node.getUserObject().equals(selectedNodeUserObject)) {
                    DefaultMutableTreeNode parentNode = (DefaultMutableTreeNode) node
                            .getParent();
                    if (parentNode.getUserObject() instanceof GameModel
                            && ((GameModel) parentNode.getUserObject())
                                    .equals(selectedReqGame)) {
                        gameTree.setSelectionPath(new TreePath(node.getPath()));
                    }
                } else if (node.getUserObject().equals(selectedNodeUserObject)) {
                    gameTree.setSelectionPath(new TreePath(node.getPath()));
                }
            }
        }
