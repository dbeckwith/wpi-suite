...
public class ViewEventController {
        ...
        overviewTable.getSelectionModel().addListSelectionListener(
                new ListSelectionListener() {
                    ...
                    public void valueChanged(ListSelectionEvent e) {
                        ...
                        toolbar.getImportExportButton().getExportButton()
    ...
    public void editIteration(Iteration iter) {
        ...
        if (exists == null) {
            ...
            main.addTab(iter.getName(), null, editIter,
                    "Editing " + iter.getName());
            main.invalidate(); //force the tabbedpane to redraw.
            main.repaint();
        ...
        for (i = 0; i < main.getTabCount(); i++) {
            if (main.getTitleAt(i).equals("Pie Chart")) {
                if (main.getTabComponentAt(i) instanceof NewPieChartPanel
                        && (((NewPieChartPanel) main.getTabComponentAt(i))
        ...
        for (int i = 0; i < main.getTabCount(); i++) {
            if (main.getTitleAt(i).equals("Bar Graph")) {
                if (main.getTabComponentAt(i) instanceof NewBarChartPanel
                        && (((NewBarChartPanel) main.getTabComponentAt(i))
                                .getTitle().equals(title))) {
                    main.setSelectedIndex(i);
                    return;
                }
    ...
    public void editRequirement(Requirement toEdit) {
        ...
        if (exists == null) {
            ...
            tabName.append(". ");
            int subStringLength = toEdit.getName().length() > 6 ? 7 : toEdit
                    .getName().length();
    ...
    public void assignSelectionToBacklog() {
        ...
        for (int i = 0; i < selection.length; i++) {
            Requirement toSendToBacklog = (Requirement) overviewTable
                    .getValueAt(selection[i], 1);
    ...
    public void editSelectedRequirement() {
        ...
        Requirement toEdit = (Requirement) overviewTable.getValueAt(
                selection[0], 1);
    ...
    public void editSelectedIteration() {
        DefaultMutableTreeNode selected = (DefaultMutableTreeNode) overviewTree
                .getTree().getLastSelectedPathComponent();
        if (selected.getUserObject() instanceof Iteration) {
            Iteration iter = ((Iteration) ((DefaultMutableTreeNode) overviewTree
                    .getTree().getLastSelectedPathComponent()).getUserObject());
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            for (File file : selectedFiles) {
                try {
                    ...
                    while ((read = fileInputStream.read(buffer)) != -1) {
                        ...
                        System.arraycopy(oldInput, 0, inputBytes, 0,
                                oldInput.length);
                        System.arraycopy(buffer, 0, inputBytes,
                                oldInput.length, read);
                    }
                    ...
                    Requirement[] importedReqs = gson.fromJson(jsonStr,
                            Requirement[].class);
            ...
            if (failed.size() > 0) {
                ...
                JOptionPane.showMessageDialog(main,
                        "Could not load all requirements: " + failStr,
    ...
    public void exportRequirements() {
        ...
        for (int i = 0; i < selected.length; i++) {
            selectedRequirements[i] = ((Requirement) model.getValueAt(
                    selected[i], 1));
        }
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            String json = new Gson().toJson(selectedRequirements,
                    selectedRequirements.getClass());
            ...
            try {
                ...
                JOptionPane.showMessageDialog(main,
                        "Could not save requirements: " + e.getMessage(),
